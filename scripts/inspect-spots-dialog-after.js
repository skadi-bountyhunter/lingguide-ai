...
async function waitChrome() {
  for (let i = 0; i < 20; i++) {
    try { return await (await fetch('http://127.0.0.1:9222/json')).json(); }
...
(async () => {
  ...
  await cdp.send('Page.navigate', { url: 'http://127.0.0.1:3001/spots' });
  await sleep(2500);
  const click = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      const btn = [...document.querySelectorAll('button')].find(b => /新增景点/.test(b.textContent || ''));
  ...
  const probe = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      if (!dlg) return { found: false, text: document.body.innerText.slice(0, 200) };
  ...
  const outFile = path.join(outDir, 'spots-dialog-after.png');
